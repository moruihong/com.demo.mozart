...
public class MusicCircleSeek extends MusicCircleProgress {
    /**
    ...
    public interface OnSeekCircleChangeListener {
         ...
         *            {@link MusicCircleProgress#setMax(int)}. (The default value for max is 100.)
         * @param fromUser
         *            True if the progress change was initiated by the user.
         */
        void onProgressChanged(MusicCircleSeek seekCircle, int progress, boolean fromUser);
         ...
         */
        void onStartTrackingTouch(MusicCircleSeek seekCircle);
         ...
         *            The SeekCircle in which the touch gesture began
         */
        void onStopTrackingTouch(MusicCircleSeek seekCircle);
    }

    private OnSeekCircleChangeListener mOnSeekCircleChangeListener;

    /**
     * Sets a listener to receive notifications of changes to the SeekCircle's progress level. Also provides
     * notifications of when the user starts and stops a touch gesture within the SeekCircle.
     *
     * @param listener
     *            The seek circle notification listener
     *
     * @see MusicCircleSeek.OnSeekCircleChangeListener
     */
    ...
    public MusicCircleSeek(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
    }

    public MusicCircleSeek(Context context, AttributeSet attrs) {
        super(context, attrs);
    }

    public MusicCircleSeek(Context context) {
        super(context);
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (updateProgress) {
            ...
            int progress = Math.round(position * mMaxProgress);

            if (event.getAction() != MotionEvent.ACTION_DOWN) {
                updateRevolutions(x, y);
                float absPosition = mRevolutions + position;
                // Clamp progress
                if (absPosition < 0.0f)
                    progress = 0;
